...
required.packages <- c("ggplot2", "data.table","R.utils", "mclust", "ClusterR")
for(reqd.packg in required.packages){
  if(!requireNamespace(reqd.packg)){
    install.packages(reqd.packg)
  }
}
...
#write.csv(resultant_for_one_column,"C:\\Users\\sh2742\\Documents\\py\\resultant_for_one_column.csv", row.names = FALSE)
